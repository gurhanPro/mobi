...
class AddParentInLaw extends Component {
  constructor(props) {
    super(props);
    this.state = {
      nationalId: '',
  ...
  handleNext2() {
    ...
    const member = {
      ...
      msisdn: phoneNumber,
      relationship: MEMBER_TYPES.PARENT_IN_LAW,
    }
  ...
  render() {
    return (<>
      <ValidatorForm onSubmit={this.handleNext2.bind(this)} key={1} style={{ padding: '5%' }}>
        ...
        <button className="next-buttons view-members-btn" type='submit' onClick={() => { this.setState({ addAnother: true }); }}>Add another parent inlaw</button>
      </ValidatorForm>
    </>
    );
  }
